		...
		if(def.style)
		{
			...
			function add_sel(s)
			{
				...
				if(s == '' || s.indexOf(prefix) == 0){
					rule += s;
				}else if((s[0] == '@' && s[1] == 'f') || (rule[0] == '@' && rule[1] == 'f')){
				//console.log(s)
				    rule += s + '' ; 
				}else if((s[0] == '@' && s[1] == 'm') || (rule[0] == '@' && s[1] == 'f')){
				//console.log(s)
				    rule += s + '' + prefix ; 
				}else { 
					rule += prefix + '' + s; 
			...
			while(pos < css.length)
			{
				...
				if(rule != ''){ 
				//console.log(s.sheet.insertRule(rule, s.sheet.cssRules.length), rule); 
				s.sheet.insertRule(rule, s.sheet.cssRules.length); rule; 
				rule = ''; }
			}
		}
